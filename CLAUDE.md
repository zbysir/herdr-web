...
- 顶栏「改动」上那个**绿点**（`hooks/useGitDirty` + `gitdiff.DirtyStat`）：判据是「有**你还没
  ...
  （探测失败不弹任何东西）。盯的是**上次在面板里看的那个仓库**，一台机器上开着几十个 pane 是
  常态（实测 34 个不同 cwd），一拍几十次 git 换一个小点不值当。
...
- 弹出组（`Key.Group` + `web/src/components/KeyGroupPopup.tsx`）：一个键在条上**只占一格**，
  ...
  粘滞 / act / 两次确认一处都别重写）。
- 软键条的宽度和钉住（`internal/softkeys`）：**宽度按内容自适应，没有「占几格」这回事。**
...
web/                  Vite + React + TS + Tailwind v4 + shadcn 风格组件
  ...
  src/components/     Dock.tsx 是底部面板的外壳（发件箱 + 软键条共用的边框 / 宽度 / 高度）
                      ...
                      DiffPanel.tsx 是改动清单（仓库从 pane 的 cwd 猜），
