...
- **chat 模式下快捷键条的键分三种**（`App.sendKeyBytes` / `viaTerm` / `whenReady`）：动作键
  （传图 / 复制…）照常；对 agent 说的键（Esc / `/clear` / 回车 / 方向）**留在 chat**、终端没连就
  连上等 herdr 起来再**补发**（不再是「连上再按一次」）—— 不用另开「往某个 pane 发键」的口，
  因为 chat 看的就是 herdr 焦点那个 pane，终端的键也落在那儿；**herdr 前缀 `^B` 开头的键 +
  粘滞 Ctrl/Alt** 在 chat 里切回终端发，按完自己回 chat（整串组合发完就算，光一个前缀等下一个
  键）。四条都是真 herdr 上踩出来的：① 「herdr 起来了」判据是**终端切进了备用屏**
  （`Session.appReady`），光看 WebSocket 连上不够 —— 那会儿对面还是 zsh，服务端正替你敲
  `herdr`；② 「人按了键」**不能用 xterm 的 `onData`**：herdr 一起来就查几百条颜色 / 焦点 /
  DSR，终端的自动回答全走 onData，拿它判断的话 ^B 还没发出去就被当成「按过下一个键了」；
  ③ 改听隐藏输入框的 keydown / input 时**必须挂捕获阶段** —— xterm 在那个框上挂的是捕获监听
  且会 stopPropagation，按现在的规则同一元素上**冒泡**阶段的监听就不触发了（一个键都收不到，
  而键照样发出去，看着像「逻辑没跑」）；④ **herdr 很多组合会弹框等人**（`^B c` 是问新 tab
  叫什么，不是直接建好；自配的跳转 / scratch 也是），所以回 chat 之前先比「屏幕上框的左上角比
  按之前多了没有」（`Session.boxCorners`），多了就等框关掉再回 —— 不维护「哪些键会弹框」的名单。
