...
- 软键的内置图标（`internal/softkeys/icons.go` + `web/src/keyicons.tsx`）：`Key.Icon` 决定
...
- 弹出组（`Key.Group` + `web/src/components/KeyGroupPopup.tsx`）：一个键在条上**只占一格**，
  ...
  粘滞 / act / 两次确认一处都别重写）。
- 软键条的宽度和钉住（`internal/softkeys` + `web/src/lib/keys.ts`）：宽度是**几格**
  （`Key.Span` 1..3，一格 = 前端 `--sk-w`，手机窄一档），不是「宽不宽」—— 整数格才谈得上
  对齐。老字段 `wide` 留成**降级镜像**（span>=2 时照旧落盘，读的时候反过来认），两个方向
  都得走通，漏一边的表现是「升级 / 降级之后我调过的宽键全变窄了」。
  **钉住**（`Pin{Left,Right}`）是「这一行头几个 / 尾几个不跟着横滑」——「呼键盘」「Esc」
  这种每隔十秒要按一次的键滑走了就等于没有。三条：① 存的是**个数**不是另一份列表，`Bar`
  照旧是那一行的**完整顺序** —— 只认 Bar 的老版本读出来是同样那些键（只是全都跟着滑），
  不会「钉住的那几个不见了」，而那是这个项目在配置上最怕的失败；② 代价是个数会随行长失效
  （别的设备删了定义 → 行变短），所以**读的时候夹住**（`resolvePin`：非负、`Left+Right ≤`
  行长、超了左边优先）——**不报错**，那不是谁的 bug，报错只会让人存不下去；③ 渲染上一行是
  三段 flex，钉住那两段 `shrink-0`，**滑动只发生在中间那段**。
  （原来这儿有个「固定块」：钉在条一端的 2×N 对齐网格。它的两个卖点现在分别被**弹出组**
  （对齐网格，一格换一片浮窗）和**钉住**（不滚动，按行、两端、不占额外地方）覆盖了，
  留着就是两个概念抢一件事，去掉了。）
