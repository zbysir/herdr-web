...
- **发件箱只管发信**：自动拉回（每拍抄远端输入框）、手动拉回（`pull`）、推草稿（「双向」）
  2026-09-24 一起去掉了（用户：「问题挺多的，只保留发信能力」，最后一根稻草是自动拉回抢焦点）。
  那一拍轮询还在，但只问「投给谁」、**不读屏**（`outbox.Where`）—— 它顺带是「人在别处切了 pane，
  chat 跟过去」的唯一信号，别整个删。**别再加回来**；非加不可先读 [OUTBOX.md](docs/dev/OUTBOX.md)
  「发件箱，不是镜像」那节（原设计就反对双向，踩过的三个 bug 都记在那儿）。
- **发件箱是一行**（`web/src/components/Compose.tsx`：输入框 + 一个投稿键，`h-8`，和快捷键条上
  ...
  拉回和双向去掉了（见上一条）、传图用顶栏那个 `img`、投给谁只剩
  ...
  框里」。④ **富输入框（contentEditable）被外面改字时，只在它本来就有焦点时才摆光标**
  （`ComposeRich` 的 `setText`）：往 contentEditable 里放选区，Chromium 会顺手把焦点挪过来
  （页面拿着窗口焦点时才会 —— 后台标签页里测不出来），当初自动拉回一抄进 agent 输入框里的字，
  就把焦点从终端手里抢走了（用户报的「chat + 终端键盘打字，切过去焦点跑到了发件箱」）。
  详见 [OUTBOX.md](docs/dev/OUTBOX.md)。
...
- **chat 模式**（`internal/transcript` + `internal/server/chatapi.go` +
  ...
  **「自己写」那一格**（TUI 的 `Type something.`，请求里的 `other`）单选和多选**走法完全不同**：
  单选按 `n+1` 就进编辑态、打字、`enter`；多选数字键只切换勾选**光标不动**，得 `down`×n 挪过去
  （打字自己会勾上，别再按 `n+1` —— 那是切换）、再 `down` + `enter` 答完这题 —— **不能用 tab**，
  光标在那一格上时 tab 只往下挪一行、到 Submit 行上 tab / right 都不动，一个字都不报。
  `down`×n 要光标在第 1 行，第一题先 `right`+`left` 进出一次（重进一页必回第 1 行；
  `home` herdr 不认、`up` 到顶会绕回去）。字里的换行 / 控制字符服务端换成空格。
  ...
  **关掉 chat = 藏起来，不卸载**（`ChatPanel` 的 `hidden`，用户点名的「每次打开都从头加载」）：
  藏着时不轮询，再打开读的是增量；用 `invisible` 不用 `display:none`（后者丢 `scrollTop`）。
  详见 [CHAT.md](docs/dev/CHAT.md) §9。
- **`herdr-web connect`**（`internal/remote` + `auth.Authenticate` 里 bearer 那段 + `auth/cliapprove.go`）：
  另一台电脑的原生终端连过来，认证是同一张设备表。三条别改：① 令牌走 `Authorization: Bearer`
  **不走 cookie** —— `/pty` 上「没 Origin 的 cookie 请求」照旧拒，那条是 SECURITY.md §7 的，
  bearer 那种 Ident **不是 Ambient**（跨站设不了这个头）；② 令牌**只在 `client:"cli"` 时进响应体**，
  浏览器那条永远只发 HttpOnly cookie；③ 断开时客户端要把**本地**终端的鼠标上报 / kitty 键盘 /
  备用屏复位（`resetModes`），不然本地 shell 里一动鼠标就灌 `35;120;36M`（网页那边同一个坑）。
  passkey 那一步借浏览器做（device flow：终端显示码 → 浏览器里**手输**、刷 passkey 批准 →
  终端轮询领走）。**到期重验只能走它，别再允许配对码重验** —— 第一版那么做过，命令行设备每天
  输个码就一直续，「长期靠 passkey」被架空。批准页是**单独的全屏页**（`CliApprovePage`，和配对页
  同一个样子），不在设置里 —— 用户点名的：这是一次「给一台电脑登录 shell」的授权，塞在设置底下一小块
  里既看不清发起方，也显得像个无关紧要的开关。码**不许从 URL 带进批准页**（`/?cli` 只翻页），
  点开就填好的链接正是钓鱼要的。详见 SECURITY.md「命令行客户端」。
- **文件查看器能改文本 + 渲染 md**（`files.WriteText` + `POST /api/files/save`、`FileViewer.tsx`）：
  ...
  别解（会弄坏文件名里真的 `%`）。
- **chat 模式下快捷键条的键分三种**（`App.sendKeyBytes` / `viaTerm` / `whenReady`）：动作键
  （传图 / 复制…）照常；**效果在对话流里看得见的键**（回车 / 「一串字 + 回车」/ Esc / ⌃C，
  `chatVisible` 那张**白名单**）**留在 chat**、终端没连就连上等 herdr 起来再**补发** —— 不用另开
  「往某个 pane 发键」的口，因为 chat 看的就是 herdr 焦点那个 pane，终端的键也落在那儿；
  **其余一律**（方向 / Tab / PgUp / 退格 / `^B` 组合 / 粘滞 Ctrl/Alt…）切到终端发、**然后留在
  终端**。**必须是白名单**（用户报的「方向键在 chat 里没任何反应」）：方向键其实发到了（真 pane 上
  ↑ 翻出了历史），只是它改的是输入框 / 菜单高亮，不进会话记录，chat 里看不见 —— 原来按黑名单只切
  `^B`，漏一个就是一个「没反应」。**别再加「按完自动回 chat」**：做过一版（等 1.2 秒 / herdr 弹框
  就等框关掉 / 人又敲键就作罢，还为它量过「屏幕上多了几个框角」），用户实测后要求去掉 ——
  「按完了没有」只有人自己知道（选菜单要连按、`^B` 之后要看结果、框里要填字），猜错一次就是画面
  在手底下被切走。**在键盘上打字是另一条规则、没有例外**（`Session.onTyping` + App 里那个 effect）：
  chat 开着而终端的键盘开着，打的**任何**键都直接进 pane，所以第一个键一出来就切到终端（用户点名
  「不光是斜杠，任何字」）—— 别把 `chatVisible` 白名单套过来。字照常发、焦点不动（键盘不收），
  终端没连就顺手连上。两条坑：① 监听必须挂**捕获阶段**、不能用 xterm 的 `onData`（herdr 启动时
  几百条自动回答全走它）；② effect 的依赖要带 `ready` —— 刷新时 chat 本来就开着，effect 第一次跑
  时 Session 还没建，之后别的依赖都不变，就一直挂不上（实测过）。
  **终端没连着时点 ⌨ / 点 `^B` 这类键：自动连，那个键转圈到连上**（`App.whenReady` 的 `linking` +
  `ui/busy.tsx`），提示**只说**「正在连接终端…」「连接成功」（用户点名要简化，别再写「连上就替你
  发出去」这种解释机制的话）。⌨ 要**当场**弹键盘（iOS 上非手势的 focus 不弹，等连上再弹弹不出来），
  所以连接期间人打的字要拦下（`Session.setHoldTyping`）—— 不拦就落进 herdr 起来之前那个 zsh，
  和服务端替人敲的 `herdr` 拼成一条。拦只能拦 **DOM 输入事件**、挂在 **host 的捕获阶段**（xterm
  自己在输入框上挂的就是捕获监听，同元素后挂的拦不住），**别拦 onData**：终端自动应答也走它，
  拦了 herdr 会卡在等回答上。用浏览器自动化验这条时注意后台标签页的定时器被节流到 ~1 秒一拍，
  `sleep(50)` 之后发的字其实已经在连上之后了 —— 看着像「漏了一个」。
  「herdr 起来了」的判据是**终端切进了备用屏**（`Session.appReady`），光看
  WebSocket 连上不够 —— 那会儿对面还是 zsh，服务端正替你敲 `herdr`。
  验这条别在默认 session 上按 ^B 组合（会动到真 tab）：开 `/<名字>` 走一个单独的
  `herdr --session`，验完 `herdr session stop / delete`。
...
internal/
  ...
  outbox/             列目标 / 解析投给谁 / 清空 / 投稿（只管发信，不读回、不推草稿）
  ...
  remote/             `herdr-web connect`：在另一台电脑的原生终端里连 /pty（配对码 → Bearer
                      令牌、raw 模式转发、断线重连、行首 `~.` 断开、断开时复位本地终端模式）
...
- **滚几屏之后零星几个字变成别的字形**（用户报的，真机确认修好了）：不是花屏也不是缺字，
  ...
  **平移所有 glyph 的页索引**，错一个那个字就从别处取纹理。**只有中文用户看得见**：一页
  ...
  治法是**到门槛就整个换一个新的 WebGL 渲染器**（`session.ts` 的 `loadWebgl` / `renewWebgl`）：
  新渲染器拿到的是一份全新的纹理集，没有任何残留状态。四条别改：① **绝不调
  `clearTextureAtlas()`** —— 第一版就是这么修的，而它在 0.19 里是坏的（读源码才看到）：
  `clearTexture()` 开头「第 0 页写入位置在 (0,0) 就返回」的短路让**第二次起每次清都是空操作**，
  `page.clear()` 又**不清每页的 `_glyphs`**，合并时拿过期的 `glyphs[0].texturePage` 去定位删哪页
  —— 删错页、平移错索引，结果比原 bug 更猛（第二次报的是大片空白 + `Ǫ`/`Ṣ`/彩色方块）；
  ② 数的是**现在有几页**（`onAdd/onRemoveTextureAtlasCanvas` 一起听）；③ 门槛按**这台 GPU**
  的合并门槛算（一半、不超过 8 —— 有的手机 `MAX_TEXTURE_IMAGE_UNITS` 就是 8）；④ 旧的 GL 上下文要
  **显式 `loseContext()`**（浏览器同时活着的上下文有上限），换的那一下用冻帧盖住。上游 0.20 beta
  已经改掉了这一族（`_evictAllPages`），但得把 xterm 本体和所有 addon 一起升到 6.1 beta。
  **桌面浏览器复现不出来**（拿 1.3 万个不同汉字灌过，旧版也不坏），验这条只能上手机。
...
- **PTY 起来（`ready`）时要把当前行列数补发一次**（`session.ts` 里 `m.t === 'ready'` 那段）。
  PTY 按 `open()` 那一刻的行列数建（写在连接 URL 上），握手期间（CONNECTING）改的尺寸被
  `applySize` 丢掉 —— 手机上正好撞上「连接中弹键盘、终端变矮」：herdr 按高的尺寸画，浏览器里是矮的，
  输入框被挤出屏幕，打的字落在状态栏那一行（用户报的，只在「第一次连上来」出现）。本地握手几毫秒，
  **本机复现不出来**，隧道上几百毫秒才撞得上。尺寸没变时 PTY 不发 SIGWINCH，无条件补发没有代价。
...
- **明暗：默认暗色，不跟浏览器偏好切**（`term/themes.ts` 的 `initialScheme`：自己选过就按选的，没选过一律暗色）。原来按 `prefers-color-scheme` 给、还挂着一个系统切换的监听，用户不要。手动切换（顶栏「明暗」、设置里那个）照旧，跟着 profile 走。离线页没有脚本读不到人选的，固定暗色。herdr 那边 `[theme] auto_switch = false`，本来也不跟。
