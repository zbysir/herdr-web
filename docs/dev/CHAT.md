...
- **在 TUI 里撤回的消息，chat 里也跟着消失 —— 但判据必须是「能证明被撤回」。** 撤回一条
  **还没被回复**的消息时，那条照旧留在文件里（`type:"user"` 实实在在写着），只是会话从它
  父亲那儿另开一支（转录是树，§4）。
  **我按「不在最后那条的链上」做过一版，把还活着的消息也删了，只能回滚** —— 排队中的消息
  挂在入队那刻的叶子上、被打断之后后面的记录自然绕过它；一条 assistant 带两个工具结果时
  那两条 `user` 也同父。所以判据改成**语义**的：**同父的几条人话里，有 assistant 后代的
  那条是活的，没有的是被撤回的**（「有回应」= 那次真发生过）。全机最近 12 个会话实测
  6 组同父人话，**每组恰好 1 条有 assistant 后代**。
  四条保守处理，宁可多留也不错杀，**全是测试用例**（`TestClaudeRetracted`）：① 没有兄弟
  不动（刚发出去那条永远没兄弟）；② 一组里一条都没被回复过时整组留着（人连发几条、agent
  还没开工 ≠ 撤回）；③ 只认 `user` 记录里的**人话** —— 排队那种是 `queued_command`
  attachment、工具结果不算人话，所以上面那两个错杀在结构上不可能；④ 后代要看有没有
  **assistant**，不能看「有没有孩子」—— 撤回那条下面仍然挂着自动附件
  （`total_tokens_reminder` 的 parent 就是它，真机上核过）。
  服务端滤掉之外还要**指名告诉前端「这几条没了」**（`Log.Gone`）：前端只会追加不会删，
  而被撤的那条早就送过去了。这和 `Updates` 是同一个形状、同一个理由 —— **证据出现在后面
  那一批里**（那条回应往往是下一次增量才读到的）。跟着来一条：**增量那一拍要多往前看一段**
  （`lookBack` 128KB）才判得出来，而那一段**只用来认撤回、不把消息并进 `Msgs`**
  （重叠送旧人话会把还没落地的投稿回显误撤掉）。
