...
claude 2.1.278 逐键试出来的，和量 codex 那个粘贴判据一个办法。结论：
	...
	带 preview 的题   序号**只把光标移过去**，还要再来一下 `enter` 才算选中（见下面那段）
	...
	例外              **只有一个问题且是单选**时，序号本身就提交了，没有 Submit 页
	                  （多补一个 `1` 会被打进输入框）。带 preview 的单题同理：
	                  序号 + `enter` 就直接提交了，也不补那一下

**带 preview 的题要多发一下 `enter`。** 选项带 `preview`（工具那边给的 ASCII 预览图）时
TUI 换成**左右分栏**：左边一列选项、右边画预览。那一屏里数字键只是把光标移过去，不选中、
也不跳题 —— 判据是屏幕最下面那行提示自己说的：
...
漏了这一下是**完全静默**的：herdr 不报错、那个口回 200、界面上说「提交成功」，而那串键从这
一题起全变成「在同一题里挪光标」——**一题都没提交**，TUI 停在原地。用户报的就是这个：「点了
提交，终端里什么都没反应，还停留在第一个问题」，而他后来是回终端自己答的。

这条一直没露头是因为**当初量协议时用的是「一道多选 + 一道单选」，两道都没有 preview** ——
而 preview **只在单选题上有**（工具那边的限制），正好和「单选发个序号就完事」撞在一起。
2026-09-22 在 herdr 里拿真 claude 2.1.278 复现并量过两种形状：

	四题全单选、第二题带 preview   `1`（A 选中并跳到 B）→ `2`（**只把光标移到 b2**）
	                              → `enter`（选中并跳到 C）→ `2` → `3` → Submit 页 `1`
	单题单选 + preview            `3`（只移光标）→ `enter` → **直接提交**，没有 Submit 页
...
### 9.7b codex 的 `/clear`：会话换了，herdr 手上那个 id 还是旧的

用户报的是「codex 里点了 clear，终端上干干净净，chat 里却还停在 clear 之前那段」。
2026-09-22 在 herdr 里拿真 codex 0.155.1 量了一遍：

	说第一句      新建 rollout `…01a0c978…`      herdr 的 agent_session = c978 ✓
	/clear        **不新建、旧文件一字节不长**    herdr 的 agent_session **还是 c978**
	再说一句      新建 rollout `…01a0c979…`      herdr 的 agent_session = c979 ✓

成因在 hook 挂的位置：herdr 装的是 `SessionStart`（`~/.codex/hooks.json` →
`herdr-agent-state.sh session`），而 codex 的 `/clear` **不触发它** —— 新会话要到第一条
消息才算开始（屏幕上那句 `To continue this session, run codex resume … (旧id)` 说的就是
旧会话已经收摊了）。

**这段窗口里数据层认不出这件事**：旧 rollout 没有任何「结束」标记、不再增长，`~/.codex`
下也没有「当前活跃会话」的记录（`.codex-global-state.json` 那几份都是陈年的）。唯一看得
出来的是屏幕，而 chat 这条路的立身之本就是不读屏（§1）。

所以**不猜**，只把事实摆出来：那条状态行上多一句「**X 分钟没更新**」（`Turn.Idle`，
服务端算的秒数，两分钟以内不说）。说下一句话之后 herdr 的 id 就更新了，chat 自己跟到
新会话上 —— 窗口只在「clear 完还没说话」这段。

claude 那边没有这条：它的 `/clear` 会换文件，而 §9.6 那条 `sig` 一变就整份丢掉的判据
正好覆盖。

### 9.8 界面上的几条

- **头上那个药丸：三档常规状态写模型名，只有「在等你」写中文**（用户点名的）。灰 / 黄 / 绿
  三档光看颜色就分得清（闲着 / 在跑 / 完成），那三个中文词是在把一格宽度花在颜色已经说过
  的话上；而「我这会儿跟哪个模型说话」原来根本没地方看 —— 左边那个 `claude` / `codex`
  小标只说得出是哪家，说不出是 Opus 5 还是 Haiku 4.5（那个小标因此去掉了）。**红的那档
  例外**：「在等你回答」是要人动手的，不能靠颜色去猜，而那时候模型名正是最不重要的信息。
  模型名从 `Turn.Model` 来（转录里 claude 记在 `message.model`、codex 记在 `turn_context`），
  美化规则是照真值定的（`claude-opus-5` → `Opus 5`、`gpt-6-sol` → `GPT 6 Sol`），
  **认不出的形状原样给出去** —— 模型 id 是别人定的，会一直变。拿不到就**退回状态词**。
...
- 思考**直接铺开**，暗一档的字 + 左边一条竖线，和正文分开（原来折成「想了一会儿（点开看）」，用户要看：盯着 agent 干活时它正是「此刻在想什么」那一段）；工具占**一行**不占气泡（一次 turn 十几条是常态）。
...
- **抢跑提示的时限要从「那一跳回来了」开始算，不是从点下去开始算。** 用户报的第三种表现：
  从 A 切到 B，先是骨架图，**然后突然冒出 A 的对话**，过一会 B 的才出来。
  链条是：`ChatPanel` 里 `active` 的推导是「抢跑那个 → 否则列表里 `focused` 那个」，时限
  在半路上到了就把抢跑清掉，而列表这会儿还说焦点在 A —— `active` 于是退回 A，**而 A 的现场
  是缓存着的**（那个 `cache`），所以它是瞬间冒出来的，等下一拍列表刷到 B 才换回去。
  从点击时刻起算等于「网络越慢越早放弃」，而那恰恰是最需要它的时候。所以 App 里数了一个
  `jumping`（还在飞的 goto 有几跳），在飞就不起算；落回 0 时 effect 重跑，时限从那一刻
  重新数。同时把 `HINT_CAP` 从 2 秒放宽到 4 秒 —— 它现在只需要盖住最后那一次往返，而那一
  份 pane 列表必然是**全量**（焦点变了，指纹一定变，11.5KB）。原来那个 2 秒是按局域网实测
  的一百多毫秒定的。
