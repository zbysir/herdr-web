...
（投给谁 / 拉回 / 图 / 双向）挪的挪、去掉的去掉，见下面各条。**它只管发信**：框里只有你自己写的字，
不会把远端输入框里的内容抄回来，也不会往远端推草稿（原来那两样和「拉回」都去掉了）。
...
→ 为什么要单独一个框、图片怎么走通、为什么去掉了同步、轮询的实测延迟：[OUTBOX.md](dev/OUTBOX.md)
...
图（png / jpg / gif / webp，按魔数认）直接看，文本原样显示，别的只能下载。**Markdown 渲染成
排好的文档**（字号跟 chat 那个设置走），里面的相对链接按这个文件所在目录解。

文本文件能**就地改**：点 ✏️ 变成一个普通输入框，改完点「保存」（或 ⌘S）。**故意不自动保存** ——
对面正跑着 agent，它也在写这些文件。保存时核一下打开之后文件有没有被改过，被改过就停下来让你
挑「重新载入」还是「照样覆盖」；有没存的修改时关掉要点两下。超过 512 KB 的（页面里只读到前半截）
和二进制不给改，也不新建文件。
...
**这块面板能挪到右边**：键区上面那行最右的小按钮，点一下在「底下 / 右边」之间切。手机横屏
推荐放右边 —— 横屏本来就没几行高，底下再压一块，终端只剩三四行。放右边时拖左边那条竖把手改
宽度，每行键横滑不换行。同样是**横屏一套、竖屏一套**（横屏放右边，转回竖屏还在底下）；手机竖屏
不出这个按钮。
...
### 在别的电脑的终端里连（`herdr-web connect`）

浏览器里的终端总有几个键抢不回来（见下面「键盘」）。另一台电脑上装了 herdr-web 的话，
可以直接在那台电脑的原生终端里连过来：

```bash
herdr-web connect https://herdr.example.com         # 默认 session
herdr-web connect https://herdr.example.com/work    # herdr --session work
herdr-web connect https://herdr.example.com --logout   # 撤销这台、删掉本地凭据
```

- **第一次登录两条路**（注册过 passkey 时会出一个小菜单，↑↓ 选、回车确定）：
  - **passkey 批准**（菜单里用 ↑↓ 选，默认就是这条）：终端里打出一个链接和一个码，在一个能用
    passkey 的浏览器里（手机就行）打开那个链接 —— 是一张单独的全屏页「批准命令行登录」—— **手输**
    那个码，看清是哪台电脑、从哪个 IP 来的，点批准、刷一次 passkey，终端那边自己就连上了。
  - **配对码**：在跑 herdr-web 的那台机器上 `herdr-web pair`，把码敲进终端。
  配出来的设备在设备页里叫「命令行 · <主机名>」，和浏览器设备一样能单独撤销。凭据存在这台电脑的
  `~/.herdr-web/remotes.json`（0600）。
- **注册过 passkey 的话，隔 `HERDR_WEB_REAUTH_HOURS`（默认 24 小时）要重验一次，只能走 passkey 批准**
  （配对码只管第一次进来）—— 和浏览器设备一样：长期用靠的是 passkey。续在原来那台设备上。
- 码**必须手输**，批准页不接受带码的链接；**只有你自己刚跑了 connect 才批准**，谁让你「帮忙输一下
  这个码」都别理。请求是从跑 herdr-web 的那台机器本身发起的时候，批准页会标出来。
- 走的是**公网口那条 https**（和浏览器同一条路），公网地址不接受 `http://`：设备令牌就是一个
  登录 shell，不能明文走。
- 断线（锁屏、换网）自己重连，重连就是新开一个 shell 再敲一次 `herdr`，屏幕和断之前一样。
  网络僵住时**行首按 `~.`** 断开（同 ssh；行首的 `~` 会晚一个键才发出去，`~/foo` 照常能敲）。
