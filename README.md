...
| Tap | With mouse reporting, sends `CSI < 0 ; col ; row M/m` (clicking panes and tabs both work) and **does not pop the system keyboard**; without it, focuses the hidden textarea (a tap there does mean "I want to type"). **It happens immediately** — only "open the link you tapped" waits, see "Double tap" |
...
| Double tap | Show / hide the system keyboard. **The first tap does not open the path or URL under it** — only that one action waits out the double-tap window (320ms) and is cancelled when a second tap arrives. Everything else (claiming herdr's `switch` button, reporting the click to herdr, focusing) happens immediately: deferring the whole lot was tried first and made every tap feel 1/3 of a second late, with opening a panel feeling stuck |
...
- **On Android the tap gesture itself re-raises the IME, and `preventDefault` cannot stop it.** After handling a GestureTap, Chromium calls `ShowVirtualKeyboard()` **unconditionally** (`WebFrameWidgetImpl::DidHandleGestureEvent`, which does not look at whether the event was cancelled), and the browser side only asks "is the currently focused element editable". And **dismissing the keyboard with the back button or a system gesture does not blur anything** — the terminal's hidden textarea is still focused, so tapping any button that deliberately keeps focus (the top bar icons, the soft keys, the 8px strip the top bar collapses into) pops the IME right back. Two fixes are in place: ① the strip **swallows `touchstart` natively** (React's `onTouchStart` is passive, so `preventDefault` there does nothing; swallowing `touchend` does not work either — "touchend handled + Android" is itself another trigger), and cancelling touchstart makes Chromium drop the whole gesture sequence, click included, so the expand action happens in that handler; ② when the viewport grows back (the keyboard is gone) while an input still holds focus, **blur it** so the app's state matches reality — the top bar comes back on its own and there is nothing editable left to raise. ② only applies on browsers that really do shrink the viewport for the keyboard (some do not move at all — see the Phones section).
