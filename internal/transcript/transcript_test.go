...
// **撤回的消息不能留在对话流里。** 人在 TUI 里按 Esc 撤回一条还没被回复的消息时，那条
// **照旧留在文件里**，只是后面的记录不再从它往下挂 —— 转录是树（`parentUuid` → `uuid`），
// 它掉出了当前分支。线性地把每行都画出来就是「TUI 里已经撤掉的消息，chat 里还在」（用户报的）。
//
// 三条钉在这儿：
//   - 撤回那条要丢掉；
//   - **判据不能是「它有没有孩子」** —— 撤回那条下面仍然挂着自动附件（真机上核过
//     `total_tokens_reminder` 的 parent 就是它），所以这儿特意给它挂一个；
//   - 比「链走到的最早那条」还早的一律保留（那些的分支关系这一窗里证不了），
//     所以正常的人话一条都不许少。
func TestClaudeDropsRetracted(t *testing.T) {
	dir := t.TempDir()
	p := filepath.Join(dir, "s.jsonl")
	var b strings.Builder
	row := func(m map[string]any) {
		m["timestamp"] = "2026-09-21T02:00:00.000Z"
		j, _ := json.Marshal(m)
		b.WriteString(string(j) + "\n")
	}
	human := func(uuid, parent, text string) {
		row(map[string]any{"type": "user", "uuid": uuid, "parentUuid": parent,
			"message": map[string]any{"role": "user", "content": text}})
	}
	agent := func(uuid, parent, text string) {
		row(map[string]any{"type": "assistant", "uuid": uuid, "parentUuid": parent,
			"message": map[string]any{"role": "assistant", "content": []map[string]any{{"type": "text", "text": text}}}})
	}

	human("u1", "", "第一句")
	agent("a1", "u1", "第一句的回复")
	// 撤回的那条：挂在 a1 下面，**而且自己下面挂着一个自动附件**（就是那个坑）
	human("u2", "a1", "撤回的那条")
	row(map[string]any{"type": "attachment", "uuid": "x1", "parentUuid": "u2",
		"attachment": map[string]any{"type": "total_tokens_reminder", "text": "<total_tokens>1</total_tokens>"}})
	// 后面这条**跳过了 u2**（parent 是 a1）—— 这就是「撤回」在树上的样子
	human("u3", "a1", "撤回之后重新说的")
	agent("a3", "u3", "这一句的回复")
	...
	want := []string{"第一句", "第一句的回复", "撤回之后重新说的", "这一句的回复"}
	got := make([]string, 0, len(l.Msgs))
	for _, m := range l.Msgs {
		got = append(got, m.Text)
	}
	if strings.Join(got, "|") != strings.Join(want, "|") {
		t.Fatalf("该是 %v，实际 %v", want, got)
	}
}

// 只有一条分支（没人撤回过）时**一条都不许少** —— 这个过滤最怕的是错杀。
func TestClaudeKeepsWholeBranch(t *testing.T) {
	dir := t.TempDir()
	p := filepath.Join(dir, "s.jsonl")
	var b strings.Builder
	prev := ""
	for i := 0; i < 8; i++ {
		...
		j, _ := json.Marshal(map[string]any{
			"type": "user", "uuid": uuid, "parentUuid": prev, "timestamp": "2026-09-21T02:00:00.000Z",
			"message": map[string]any{"role": "user", "content": "第 " + itoa(i) + " 句"},
		})
		b.WriteString(string(j) + "\n")
		prev = uuid
	}
	...
	if len(l.Msgs) != 8 {
		t.Fatalf("该有 8 条，实际 %d：%v", len(l.Msgs), brief(l))
	}
}

// **撤回的证据出现在后面那一批里，所以增量那一批必须报 `Gone`。**
//
// 人按 Esc 撤回一条还没被回复的消息时，那条照旧留在文件里；「后面的记录跳过它」这条证据
// 往往是**下一次增量**才读到的 —— 那时候它早就送到浏览器里了，而前端只会追加不会删。
// 所以这儿钉两件：第一批把它当正常消息送出去（那时候确实证不了），第二批把它报进 Gone。
func TestClaudeGoneInIncremental(t *testing.T) {
	dir := t.TempDir()
	p := filepath.Join(dir, "s.jsonl")
	row := func(m map[string]any) string {
		m["timestamp"] = "2026-09-21T02:00:00.000Z"
		j, _ := json.Marshal(m)
		return string(j) + "\n"
	}
	human := func(uuid, parent, text string) string {
		return row(map[string]any{"type": "user", "uuid": uuid, "parentUuid": parent,
			"message": map[string]any{"role": "user", "content": text}})
	}
	first := human("u1", "", "第一句") + human("u2", "u1", "会被撤回的那条")
	...
	// 第一批：两条都在，还没有 Gone（这时候确实证不了）
	if len(a.Msgs) != 2 || len(a.Gone) != 0 {
		t.Fatalf("第一批该是 2 条 + 0 个 Gone，实际 %d 条 + %v", len(a.Msgs), a.Gone)
	}

	// 撤回：后面那条**跳过 u2**，parent 还是 u1
	extra := human("u3", "u1", "撤回之后重新说的")
	...
	b, err := Read(src, a.Next)
	if err != nil {
		t.Fatal(err)
	}
	if len(b.Gone) != 1 || b.Gone[0] != "u2" {
		t.Fatalf("增量那批该报 Gone=[u2]，实际 %v（msgs=%v）", b.Gone, brief(b))
	}
	// 新说的那条要正常送出来
	if len(b.Msgs) != 1 || b.Msgs[0].Text != "撤回之后重新说的" {
		t.Fatalf("增量那批该只有新说的那条，实际 %v", brief(b))
	}
}
