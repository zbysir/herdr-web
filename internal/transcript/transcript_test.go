...
**判据是语义的，不是结构猜测**：同父的几条人话里，**有 assistant 后代的那条是活的，
没有的是被撤回的**（全机最近 12 个会话实测 6 组同父人话，每组恰好 1 条有 assistant 后代）。

我先前按「不在最后那条的链上」做过一版，**把还活着的消息也删了**，只能回滚。所以这儿
**先把那几类错杀写成用例**，它们比「撤回能不能认出来」更要紧：

	排队中的消息        挂在入队那刻的叶子上，被打断之后后面的记录自然绕过它
	一条 assistant 两个工具结果  那两条也是 `user` 记录、也同父
	刚发出去那条        后面还没有任何记录
	连发几条还没开工    一组里一条都没被回复过
*/
func TestClaudeRetracted(t *testing.T) {
	type row = map[string]any
	mk := func(rows ...row) string {
		var b strings.Builder
		for _, r := range rows {
			r["timestamp"] = "2026-09-21T02:00:00.000Z"
			j, _ := json.Marshal(r)
			b.WriteString(string(j) + "\n")
		}
		return b.String()
	}
	human := func(uuid, parent, text string) row {
		return row{"type": "user", "uuid": uuid, "parentUuid": parent,
			"message": row{"role": "user", "content": text}}
	}
	asst := func(uuid, parent, text string) row {
		return row{"type": "assistant", "uuid": uuid, "parentUuid": parent,
			"message": row{"role": "assistant", "content": []row{{"type": "text", "text": text}}}}
	}
	attach := func(uuid, parent string) row {
		return row{"type": "attachment", "uuid": uuid, "parentUuid": parent,
			"attachment": row{"type": "total_tokens_reminder", "text": "<total_tokens>1</total_tokens>"}}
	}
	queued := func(uuid, parent, prompt string) row {
		return row{"type": "attachment", "uuid": uuid, "parentUuid": parent,
			"attachment": row{"type": "queued_command", "humanTurn": true, "prompt": prompt}}
	}
	toolUse := func(uuid, parent, id1, id2 string) row {
		return row{"type": "assistant", "uuid": uuid, "parentUuid": parent,
			"message": row{"role": "assistant", "content": []row{
				{"type": "tool_use", "id": id1, "name": "Bash", "input": row{"command": "ls"}},
				{"type": "tool_use", "id": id2, "name": "Bash", "input": row{"command": "pwd"}},
			}}}
	}
	toolRes := func(uuid, parent, id string) row {
		return row{"type": "user", "uuid": uuid, "parentUuid": parent,
			"message": row{"role": "user", "content": []row{{"type": "tool_result", "tool_use_id": id, "content": "ok"}}}}
	}
	...
	for _, tc := range []struct {
		...
		{
			"撤回：同父两条人话，只有后一条拿到了回应（撤回那条下面还挂着自动附件 —— 所以不能靠「有没有孩子」判）",
			mk(human("u1", "", "第一句"), asst("a1", "u1", "回复一"),
				human("u2", "a1", "撤回的"), attach("x1", "u2"),
				human("u3", "a1", "重新说的"), asst("a3", "u3", "回复二")),
			[]string{"第一句", "回复一", "重新说的", "回复二"},
		},
		{
			"**排队中的消息不许删**：它是 attachment，被打断后后面的记录会绕过它",
			mk(human("u1", "", "第一句"), asst("a1", "u1", "回复一"),
				queued("q1", "a1", "排队那条"),
				human("u2", "a1", "后来发的"), asst("a2", "u2", "回复二")),
			[]string{"第一句", "回复一", "排队那条", "后来发的", "回复二"},
		},
		{
			"**一条 assistant 两个工具结果不许删**：那两条也是 user 记录、也同父",
			mk(human("u1", "", "跑两条命令"), toolUse("a1", "u1", "t1", "t2"),
				toolRes("r1", "a1", "t1"), toolRes("r2", "a1", "t2"),
				asst("a2", "r2", "跑完了")),
			[]string{"跑两条命令", "Bash", "Bash", "跑完了"},
		},
		{
			"**刚发出去那条不许删**：后面还没有任何记录",
			mk(human("u1", "", "第一句"), asst("a1", "u1", "回复一"), human("u2", "a1", "刚发的")),
			[]string{"第一句", "回复一", "刚发的"},
		},
		{
			"**连发几条、agent 还没开工时整组留着**（一条都没被回复过 ≠ 撤回）",
			mk(human("u1", "", "第一句"), asst("a1", "u1", "回复一"),
				human("u2", "a1", "连发一"), human("u3", "a1", "连发二")),
			[]string{"第一句", "回复一", "连发一", "连发二"},
		},
	} {
		dir := t.TempDir()
		p := filepath.Join(dir, "s.jsonl")
		...
		got := make([]string, 0, len(l.Msgs))
		for _, m := range l.Msgs {
			if m.Kind == KindTool {
				got = append(got, m.Tool)
			} else {
				got = append(got, m.Text)
			}
		}
		if strings.Join(got, "|") != strings.Join(tc.want, "|") {
			t.Errorf("%s\n  该是 %v\n  实际 %v", tc.why, tc.want, got)
		}
	}
}
