...
export interface ChatLog {
  ...
  /**
   * **前面送过、但现在证明已经被撤回的那几条**（消息 id）。
   *
   * 和 `updates` 同一个理由：证据出现在后面那一批里。人在 TUI 里按 Esc 撤回一条还没被
   * 回复的消息时，那条**照旧留在转录里**，只是后面的记录不再从它往下挂（转录是树）——
   * 而那条「跳过它」的记录往往是下一次增量才读到的，这时候它早就送到浏览器里了。
   * 前端只会追加、不会删，所以必须按这个名单去掉，不然屏幕上那条一直挂着，
   * 只有整份重读（换会话 / 重开面板）才消失。
   */
  gone?: string[]
  agent: string
