...
/** 键那一区的高度。语义没变，沿用快捷键条时代的键名，已经调好的高度不会因为这次合并丢掉 */
 ...
 * 连选字都选不了。快捷键条时代把手压在按钮上还能忍（按钮点哪儿都一样），到了输入框
 * 就不行了。
 */
const EDGE_W = 16
...
/**
 * 底部面板：发件箱 + 快捷键条装在**同一块**里。
 *
 * 以前这是两块各管各的：发件箱能抓着 ⠿ 从底部「撕」下来变成浮动面板（自己一套位置 /
 * 大小），快捷键条自己一套左右留白和高度。两块叠在一起时是两条边框、两种宽度、两套把手 ——
 ...
 *
 * **手机竖屏（< PHONE_MAX）整套把手都不出**，面板通屏、快捷键条**一行横滑**：那么窄的
      ...
      >
        ...
        {keys && (
          <>
              ...
              >
                {(['l', 'm', 'r'] as const).map((zone) => (
                  <span
                    ...
                    title={
                      zone === 'm'
                        ? '拖我：上下改快捷键条高度（最多半屏），左右整条平移。双击复位'
                        : `拖我：上下改快捷键条高度（最多半屏），左右改${zone === 'l' ? '左' : '右'}边界。双击复位`
                    }
            ...
            {/* 键区。**横向**怎么排是快捷键条自己的事（它分两行，各自横滑，见 Softkeys）；
