 ...
 *
 * 为什么摆在**分页条上面**而不是单开一页：下面「顶栏」和「快捷键条」两页改的就是这一套里的
 * 东西，而「我正在改哪一套」是看这两页时必须一直看得见的。塞进第五页的话，人在快捷键条页上
 * 拖了十分钟才发现改的是手机那套。
 *
 * 换一套 = 把**这台设备**绑过去（服务端记着，见 internal/profiles）。**不按屏幕宽度自动
 * 切**：分屏、转屏、外接屏都会让宽度跳变，而这里面装的正是快捷键条那种一跳就手忙脚乱的东西。
 *
    ...
    <div className="mb-1 flex flex-col gap-1.5">
      ...
      <p className="text-xs/relaxed text-faint [&_strong]:font-medium [&_strong]:text-muted">
        {editing === 'new' ? (
          ...
          <>
            「顶栏」「快捷键条」两页改的就是<strong>这一套</strong>，换一套整条换掉；
