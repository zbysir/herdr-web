...
/**
 * 「顶到右边界」容几格空白。**不是 0** —— TUI 在自己那一层还会让出几列，而且**每个
 * TUI、每种块让的还不一样多**，所以这个数是拿真机量出来的，不是推出来的：
 *
 *   - Claude Code（Ink）42 列的 pane：herdr 的顶栏铺满 0..41 列，而消息框的字最多画到
 *     第 39 列 —— 差两格。
 *   - codex（ratatui）51 列的 pane：列表项那一行（`• /Users/…/art-`）离带的右边界**差
 *     四格** —— 它自己那层还留了滚动条的一列、列表块又有右留白。用户报的就是这一条。
 *
 * 少一格的后果**不是「少一条链接」，而是一条指错地方的链接**：折行没认出来的话，续行
 * 「`assets/chapter-7/pianist-nochair.png`」自己就是一条合法的相对路径（带扩展名），于是
 * 按焦点 pane 的 cwd 解出 `…/recursion-game/assets/…`（真路径是 `…/art-assets/…`）——
 * 屏幕上那条下划线看着完全正常，点下去报「找不到」，而人是照着屏幕点的。
 *
 * 放宽到四格靠的是**另外三条判据兜底**（下面 tuiWrap 里那四条）：这条只回答「这一行还有
 * 没有地方」，真正挡住「正常断句刚好停在边界附近」的是第 3 条（被切开的那个词得真的一行
 * 装不下）和第 4 条（结尾看着已经是个完整文件名就别拼）。
 */
const EDGE_SLACK = 4
