...
→ 为什么要单独一个框、图片怎么走通、双向同步的注意事项、轮询的实测延迟：[OUTBOX.md](OUTBOX.md)
...
- `act:` 是网页端动作，不发字节：`act:kbd` 呼出键盘、`act:img` 传图、`act:panes` 开面板一览、
  `act:files` 开文件、`act:clip` / `act:paste` 是[手机上的复制粘贴](MOBILE.md#手机上怎么复制--粘贴)。
...
→ 排序规则、「3 分钟前」那一列、什么时候弹、红点怎么算、系统通知：[MOBILE.md](MOBILE.md)
　那段话是怎么抽出来的：[COMPOSER.md](COMPOSER.md)
...
→ 短时链接那条路、四条硬规矩（绝不以 `text/html` 吐、SVG 为什么敢渲染）：[SECURITY.md](SECURITY.md)
...
| 单击 | 有鼠标上报时发 `CSI < 0 ; col ; row M/m` 给程序（点 pane、点 tab 都好使）且**不弹系统键盘**；没有鼠标上报时聚焦隐藏 textarea（点一下就是想打字）。**发出去要等过双击窗口（320ms）**，见下面「双击」；而网页自己的动作（接走 herdr 顶栏那个 switch 开面板一览）当场就做 |
| 长按（≈380ms）| **抓住**：按下左键不松手 + `CSI < 32` 上报移动，之后滑动就是拖 —— herdr 的「拖 pane 边框改大小」在手机上靠这条。松手补 `m` 发松开 |
| 双击 | 显示 / 收起系统键盘。**第一下什么都不留** —— 凡是「出得去、收不回」的（鼠标上报发给 pane 里那个程序、打开点到的路径 / URL）都攒到双击窗口（320ms）过去才做，来了第二下整套撤掉。界线是「这件事出得去吗」：出不去的（开我们自己的面板）立刻做，不然点什么都像卡住。为什么必须这么严：Claude Code 自己有可点的东西（展开一段、**选一个选项**），双击的第一下漏出去就把选项给选了 |
...
→ 手势为什么这么分、键盘怎么收、手机上怎么复制粘贴、面板和顶栏的细节：[MOBILE.md](MOBILE.md)
...
见 [MOBILE.md](MOBILE.md#手机上怎么复制--粘贴)。
...
→ 威胁模型、每条为什么这么设计、还没做的：[SECURITY.md](SECURITY.md)
...
| 要看什么 | 去哪儿 |
|---|---|
| 发件箱：为什么单独一个框、图片怎么走通、轮询实测 | [OUTBOX.md](OUTBOX.md) |
| 读屏：抽输入框、抽提示卡上那段话 | [COMPOSER.md](COMPOSER.md) |
| herdr socket API 实测出来的语义 | [HERDR-API.md](HERDR-API.md) |
| 手机 / 平板那一整套（手势、键盘、面板、顶栏、提示、复制粘贴） | [MOBILE.md](MOBILE.md) |
| 安全设计和威胁模型、文件浏览那条路的规矩 | [SECURITY.md](SECURITY.md) |
