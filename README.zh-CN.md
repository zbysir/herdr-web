...
分页条**上面**还有一行：**这台设备用哪一套排布**（下面那节）—— 而「终端」页上的每一项都属于那一套。顶栏最右的 ⚙ 是全部设置，分四页：**终端**（字号 / 明暗、kitty 协议 / Option 当 Meta / 选中即复制 / 同步输出、点 herdr 的 switch 开面板一览、面板图标上的红点，加一行后端环境）、**顶栏**（下面那节）、**软键条**（再下面那节）、**设备**（谁配过对、登出、踢人）。

「终端」页顶上那排**字号 / 明暗**和顶栏里那两三个图标是同一套动作（不是另一份状态）：顶栏上那几个是可以拖掉的，拖掉之后这儿仍然调得到。

这块面板**没有标题栏**：✕ 挪到了分页条最右边。分页条是粘在顶上的，滚到哪儿关闭都在手边，而一行「设置」标题占的 44px 对软键条那一页更值钱。所有从顶栏呼出的面板现在都**贴着顶栏 6px**，不再是 44px（窄屏 100px）—— 那一截空白什么都不显示，白吃掉的恰恰是最缺高度的那两页。

三块面板（面板一览 / 文件 / 设置）是**互斥**的：它们都浮在同一个角上，同时开两块就是互相遮盖。实现上是**一个 `panel` 字段存「当前是哪一块」**，不是三个布尔量 —— 一个字段的话第二块压根**存不下**，而三个布尔量要靠每个入口都记得把另外两个关掉，而入口不止顶栏（软键条的 `act:`、herdr 的 switch、提示卡的「更多」、终端里点一条路径都能开），漏一个就是一个只在某条路径上冒出来的遮挡。**查看器是刻意的例外**：它压在文件面板上面，关掉之后还回到原来那个目录。
...
手机没有 Ctrl 键，herdr 的 `ctrl+b` 前缀全靠这条。按键**存在服务端**（`~/.herdr-web/softkeys.json`），在设置 →「软键条」页改：**定义**所有设备共用一份，而**几行、哪些键在条上是按套分的**（见[排布分套](#排布分套一类设备一套)）。

**一行还是两行是个设置**（存服务端，是那一套的一部分），不是靠「第二行空不空」猜 —— 空的第二行和「我只要一行」是两件事。两行**各自横向滚动**：手机上一行放得下四五个键，把常用的放第一行、次常用的放第二行，比十几个键排成一条长龙好找 —— 手指知道自己在哪一行，滑一行也不会把另一行带跑。切回一行时第二行的键会**并到第一行末尾**（服务端也是这么算的；「存着但不显示」是最烦人的一种状态）。
