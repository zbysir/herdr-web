...
| 单击 | 有鼠标上报时发 `CSI < 0 ; col ; row M/m` 给程序（点 pane、点 tab 都好使）且**不弹系统键盘**；没有鼠标上报时聚焦隐藏 textarea（点一下就是想打字）。**当场就做，没有延迟** —— 只有「打开点到的那条链接」会等，见下面「双击」|
| 长按（≈380ms）| **抓住**：按下左键不松手 + `CSI < 32` 上报移动，之后滑动就是拖 —— herdr 的「拖 pane 边框改大小」在手机上靠这条。松手补 `m` 发松开 |
| 双击 | 显示 / 收起系统键盘。**第一下不会顺手把点到的路径 / URL 打开** —— 只有「开链接」这一件事攒到双击窗口（320ms）过去才做，来了第二下就撤掉。别的（接走 herdr 顶栏那个 switch、上报给 herdr、聚焦）一律当场就做：一开始我把整套都推迟了，结果点什么都慢 1/3 秒，开面板像卡住 |
