...
启动后会把**进来的三条路**各打一行：**本机**（`127.0.0.1`）、**局域网**（每张网卡一行，
按打分排序，手机真能连上的那个标成 `← 手机用这个`，虚拟网卡会标出来）、**远程**
（`HERDR_WEB_PUBLIC_URL`）。哪条没开就写清楚缺哪一样 —— 「没打印」和「没有」在屏幕上
长得一样。二维码编的是配对链接（有公网地址就用公网那个）。
...
页面底下那条输入框就是发件箱，顶栏 ✎ 开关，**默认开着**。在里面说话打字，改完整段投进
herdr 的某个 pane。

它**只有一行**（和快捷键条上一个键一样高）—— 手机上屏幕本来就是借给终端的，原来那几排控件
（投给谁 / 拉回 / 图 / 双向）挪的挪、进设置的进设置，见下面各条。

| 操作 | 说明 |
|---|---|
| **目标** | 投给你此刻在 herdr 里激活的那个 pane，不用选。换目标去「面板一览」点一下；**框里一有你自己写的字，目标就锁在当初那个 pane 上**（框空了才重新跟随） |
| **投稿** | 默认**按回车就投**（软键盘上的、快捷键条上的 `↵`、真键盘上的都算），换行用 `⇧↵`；`⌘↵` / `Ctrl↵` 一直是投稿。想让回车换行：设置 →「终端」→ 发件箱回车 |
| **自动拉回** | 默认 500ms 一拍。切了 pane 自动换成新 pane 的内容；**本地有草稿时绝不覆盖** |
| **拉回** | 把远端输入框里已有的内容抓进来编辑（远端按过 Tab 补全就用它）。现在是**动作库里的一件事**，要用就在设置 →「顶栏」/「快捷键条」里加一个「拉回」 |
| **双向** | 本地改动跟着推回远端输入框（不回车）。**默认关**，开关在设置 →「终端」，注意事项见下面 |
| **传图** | 顶栏 / 快捷键条上的「传图」（手机上给「相机 / 相册」），传完把路径接到草稿末尾。电脑上截完图直接在框里 `⌘V`，或者把文件拖进框里 —— 那两种插在**光标处** |
| `↑` | 框空时取回上一条投过的（本地留 30 条） |
| `Esc` | **转发给终端**。Esc 在纯 textarea 里没意义，而 agent 那边到处要用它（`/usage` 之类浮层靠它退出）；焦点不动，可以连按 |

「这会儿投给谁、那边什么状态」写在框空着时的提示文字里（原来是单独一行）。
...
（`~/.herdr-web/softkeys.json`），手机 / 平板 / 电脑共用一份定义，在设置 →「快捷键条」页改。
...
（摊在条上要 3×2 六格，手机竖屏上那是半条屏幕）。点「快捷键条」页上的**「方向键」**一下就摆好。
...
**方向键按住不放会连发**（↑ ↓ ← → 和 PgUp / PgDn，条上和顶栏上都是）：按住约 0.4 秒开始 ——
翻历史命令、在 agent 的选择列表里往下挑，不用点十下。手指往旁边一划（横滑找键）就不算按住，
不会白发。回车、`⌃C`、退格**有意不连发**：多发一次就是多跑一条命令、或者吃掉一整行，而终端里
没有撤销。

**速度在设置 →「终端」页调**（「方向键长按」那一行）：`16 / 8 / 4 / 2 / 1` 次每秒，默认 16
（物理键盘那一档）。触屏上按下到抬起本来就比按键盘长，16 常常一按就冲过头 —— 想下移两项结果
走到底；慢的那几档是「走得动、但看得住」。它跟着**这一套排布**走（手机一套、平板一套）。
...
顶栏的 ▦（或快捷键条上的 `act:panes`，手机上还能点 herdr 自己顶栏那个 `switch`）打开一张 pane
列表：一行一个，**点一下就跳过去并铺满全屏**。上面能筛（tab / 标题 / 路径 / pane id）、能只看
跑着 agent 的。列表自己 4 秒刷一次。

agent 停下来等你回答（或刚跑完），顶栏 ▦ 上点一个红点；开一次面板一览就算看过，红点才灭。

想让它**主动叫你**（右上角弹一张卡片，点卡片 = 跳过去）：设置 →「终端」页的「右上角弹提示卡」，
**默认关**。那几张卡浮在终端右上角，盖住的正是 agent 刚写的那几行 —— 而红点和面板一览里那列
「几分钟前」不打断，一直开着。卡片停留多久（5 秒 / 12 秒 / 30 秒 / 1 分钟 / 一直挂着）在它下面调；
「等你回答」那种一直挂着，不受这个管。

卡上**只报「谁、什么事」，不贴那段话**（系统通知同理）。原来是贴的，去掉的理由是三点叠在
一起：读屏抽出来的话经常不准（从 TUI 画面上扒的），卡上那点地方本来也放不全，而**反正每次
都要点进去看** —— 那五行的代价就是白付的，它压住的正是终端右上角那几行。
...
键盘现在只走**按钮**：快捷键条上的 ⌨（`act:kbd`，出厂配置第一个键就是它，手机上快捷键条默认开着）
和顶栏上那个「系统键盘」（在设置 →「顶栏」里拖上去）。点了就是要键盘 —— 不用猜，也没有延迟。

**发件箱和快捷键条是同一块底部面板**：两条侧边左右拖改宽度（输入法压住半边屏幕时把整块缩到
剩下的空地上），键那一区上边缘三个把手上下拖改高度、左右拖改边界，任意把手双击复位。
手机竖屏（< 440px）自动换一档：把手全收、面板通屏、快捷键条一行横滑。**横屏一套、竖屏一套**，
转屏就换成那一份。
...
装完是**独立窗口**：没有地址栏和工具条（等于白送好几行终端）、桌面 / 主屏上有自己的图标、
从任务切换器里单独出现，`⌘W` `Ctrl+W` 这类被浏览器吃掉的键也能拿回一部分。

| 平台 | 怎么装 |
|---|---|
| Android（Chrome / Edge）| 菜单 → **「安装」**。菜单里会有两档，**「创建快捷方式」不是它** —— 那个装出来的还是个开浏览器的书签 |
| 桌面 Chrome / Edge | 地址栏右边那个安装图标，或菜单 →「安装 herdr-web」 |
| iPhone / iPad | 分享 → **添加到主屏幕**（Safari 没有「安装」这个说法）|

**前提是「证书有效的 https」**，这条最容易卡住：

- ✅ 有域名 + 真证书（上面配法 3 和 4，`HERDR_WEB_TLS=acme` 或 `proxy`）—— 正常出现。
- ✅ `http://localhost` / `http://127.0.0.1` —— 浏览器把本机当安全上下文，也能装。
- ❌ **自签证书**（`HERDR_WEB_TLS=auto`，局域网直连那条）—— 点过「继续访问」也不行：
  能用、能存密码，但浏览器**不给装**，菜单里就只剩「创建快捷方式」。要在局域网里装，
  得把 `~/.herdr-web/` 下那个本地 CA 导进手机的系统信任。
- ❌ 明文 `http://` 的局域网地址 —— 同上，而且系统通知也一起没有。

iOS 上有两件事要按顺序做：**先添加到主屏幕，再在里面扫码配对**（主屏 web app 的存储容器和
Safari 是分开的，Safari 里配好的凭据带不过去）；系统通知也只有从主屏打开才拿得到权限。

离线打不开是正常的 —— 这是个终端，没有服务端就没有内容可看；连不上时给的是一张说明页
（在哪断的、怎么回事），不是浏览器的恐龙。

### 设置面板

顶栏最右的 ⚙，分四页：**终端**（字号 / 明暗 / 主题色、kitty 协议 / Option 当 Meta /
选中即复制 / 同步输出、按键样式 / 弹窗透明度 / 方向键长按、发件箱回车、提示卡和红点）、
**顶栏**、**快捷键条**、**设备**。
分页条上面还有一行「这台设备用哪一套排布」。三块浮层（面板一览 / 文件 / 设置）是互斥的。

**主题色**内置六套（绿 / 蓝 / 紫 / 青 / 品红 / 石墨），点一下当场换，终端里的光标和选区
也跟着走。换的只是界面上那一点强调色 —— 灰阶和「等你」（红）/「在跑」（黄）那几个状态色
一个都不动，终端里程序自己输出的颜色更是碰都不碰。和别的设置一样**跟着这一套排布走**，
所以手机上一套、电脑上另一套是可以的。
