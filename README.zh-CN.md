...
- 每个键的**「宽」是几格**（1 / 2 / 3）：一格就是一个键位那么宽。用格数而不是「宽不宽」，
  是为了**跨行对得齐** —— 一个 2 格的键和它上面两个 1 格的键左右边缘正好对上。
- 每个键可以挑一个**内置图标**（键盘 / Esc / Ctrl / 关闭 / 放大缩小 / 方向……三十多个），
  条上就画图标不画字。名字还留着，指上去能看到。`⌨` 这类字形在很多字体里缺（显示成方框）、
  难看、基线还跟旁边的字母对不齐，图标是 SVG，三个问题一起没了。
...
**固定块**（「软键条」页上的「固定块」那一行）是钉在条一端的一小片**对齐网格**，
**不跟着横滑**：
...
可选列数 1–4、钉左还是钉右；往空格（`·`）上拖就是放进那一格，块内两格互拖是**对调**。
里面放一个**弹出组**最省地方 —— 一个永远不滑走的格子，点开是方向键。
它是**排布**，所以每套一份（平板摆 4 列、手机竖屏摆 3 列各存一份），格子里放的还是
「我的按键」那些全局定义。为什么必须单独做一块（而不是把 `← ↓ →` 摆到 `↑` 底下）：
[MOBILE.md](docs/dev/MOBILE.md#键宽是几格固定块钉在一端不滑)。
